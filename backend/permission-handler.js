...
var trelloRecordNameRegex = new RegExp( /^trello\/.*/ );
...
PermissionHandler.prototype.canPerformAction = function( username, message, callback ) {
	 ...
	 */
		...
		var isAllowed = !trelloRecordNameRegex.test( messageObj.name ) || !messageObj.isChange;
		var errorMessage = isAllowed ? null : 'Can\'t update trello cards from client';
		callback( errorMessage, isAllowed );
	}
};

var providers = [ 'trello-provider' ];
